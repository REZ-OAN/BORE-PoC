...
impl Client {
    ...
    pub async fn new(local_port: u16, to: &str, port: u16) -> Result<Self> {
        // connection to the server control port
        let stream = TcpStream::connect((to, CONTROL_PORT)).await?;
        let mut stream = BufReader::new(stream);

        // sends message from the server, containing  the assigned remote port
    ...
    pub async fn listen(mut self) -> Result<()> {
        ...
        loop {
            ...
            match msg {
                ...
                Some(ServerMessage::Connection(id)) => {
                    ...
                    // Spawns a new asynchronous task for each incoming connection
                    tokio::spawn(
                        async move {
                            info!("new connection");
        ...
        // Handles individual connections received from the server.
        // Establishes TCP connections to the local and remote ports using TcpStream::connect.
        // Sends a ClientMessage::Accept message to the server, acknowledging the connection.
        // Forwards data between the local and remote connections,
        // using the proxy function defined in the shared module.
